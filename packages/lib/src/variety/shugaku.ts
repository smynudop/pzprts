...
//
export const Shugaku = createVariety({
	pid: "shugaku",
	//---------------------------------------------------------
	// マウス入力系
	MouseEvent: {
		...
		inputFuton: function () {
			...
			if (this.inputData <= 3) {
				...
				let old = this.inputData, adj = null;
				if (cell.isnull) { /* nop */ } // 何もしない
				else if (this.mouseCell === cell) { this.inputData = 41; } // 入力開始時と同じセルの場合
				else {
					const dx = (this.inputPoint.bx - this.firstPoint.bx), dy = (this.inputPoint.by - this.firstPoint.by);
					const adc = this.mouseCell.adjacent;
		...
		inputFuton2: function () {
			...
			const adj = this.currentTargetADJ();
			if (!adj.isnull) {
				this.changeHalf(adj);
				adj.setQans({ 42: 48, 43: 47, 44: 50, 45: 49 }[this.inputData as 42 | 43 | 44 | 45]);
				adj.setQsub(0);
			}
		...
		changeHalf: function (cell: Cell) {
			let qa = cell.qans, adc = cell.adjacent, adj = null;
		...
		currentTargetADJ: function (): Cell {
	...
	Cell: {
		...
		isPillow: function (): boolean { return (this.qans >= 41 && this.qans <= 45); }
	},
	Border: {
		group: "border",
		// 41～45:まくらありふとんマス 46～50:まくらなしふとんマス
		// 42/47,43/48,44/49,45/50:それぞれ上・下・左・右だけ境界線がない
		isbdh_cc1: { 41: 1, 42: 1, 44: 1, 45: 1, 46: 1, 47: 1, 49: 1, 50: 1 } as Record<number, number>,
		isbdh_cc2: { 41: 1, 43: 1, 44: 1, 45: 1, 46: 1, 48: 1, 49: 1, 50: 1 } as Record<number, number>,
		isbdv_cc1: { 41: 1, 42: 1, 43: 1, 44: 1, 46: 1, 47: 1, 48: 1, 49: 1 } as Record<number, number>,
		isbdv_cc2: { 41: 1, 42: 1, 43: 1, 45: 1, 46: 1, 47: 1, 48: 1, 50: 1 } as Record<number, number>,

		isBorder: function (): boolean {
			const qa1 = this.sidecell[0].qans;
			const qa2 = this.sidecell[1].qans;
	...
	BoardExec: {
		adjustBoardData: function (key, d) {
			let trans: Record<number, number> = {};
			switch (key) {
			...
			for (let i = 0; i < clist.length; i++) {
				const cell = clist[i], val = trans[cell.qans];
	...
	Graphic: {
		...
		getBGCellColor_error1: function (cell): string | null {
		...
		drawFutons: function () {
			let g = this.vinc('cell_futon', 'crispEdges', true), mv = this.puzzle.mouse, tc = null, adj = null;

			const inputting = (!mv.mouseCell.isnull && mv.firstPoint.bx !== null && mv.inputData >= 40);
			if (inputting) { // ふとん入力中
				tc = mv.mouseCell;
				//@ts-ignore
			...
			for (let i = 0; i < clist.length; i++) {
				let cell = clist[i], isdraw = (cell.qans >= 41);
				let color = (cell.error === 1 ? this.errbcolor1 : "white");
				if (inputting) {
		...
		drawPillows: function () {
			let g = this.vinc('cell_pillow', 'crispEdges', true), mv = this.puzzle.mouse, tc = null, adj = null;

			const inputting = (!mv.mouseCell.isnull && mv.firstPoint.bx !== null);
			if (inputting) { // ふとん入力中
				tc = mv.mouseCell;
				//@ts-ignore
				adj = mv.currentTargetADJ();
			...
			const clist = this.range.cells;
			const rw = this.bw * 0.7 - 1, rh = this.bh * 0.7 - 1;
			for (let i = 0; i < clist.length; i++) {
				let cell = clist[i], isdraw = (cell.qans >= 41 && cell.qans <= 45);
				if (inputting) {
		...
		getBorderColor: function (border): string | null {
			let isdraw = border.isBorder(), mv = this.puzzle.mouse, trial = false;
			const cell1 = border.sidecell[0], cell2 = border.sidecell[1];

			if (!mv.mouseCell.isnull && mv.firstPoint.bx !== null) { // ふとん入力中
				//@ts-ignore
				const tc = mv.mouseCell, adj = mv.currentTargetADJ();
				const istc = (cell1 === tc || cell2 === tc);
				const isadj = (cell1 === adj || cell2 === adj);
	...
	Encode: {
		...
		decodeShugaku: function () {
			...
			for (i = 0; i < bstr.length; i++) {
				...
				if (ca >= '0' && ca <= '4') { cell.qnum = Number.parseInt(ca, 36); }
				else if (ca === '5') { cell.qnum = -2; }
				else { c += (Number.parseInt(ca, 36) - 6); }
	...
	FileIO: {
		decodeData: function () {
			this.decodeCell(function (cell, ca) {
				...
				else if (ca >= "a" && ca <= "j") { cell.qans = Number.parseInt(ca, 20) + 31; }
	...
	AnsCheck: {
		...
		checkFutonAisle: function () {
			...
			for (let c = 0; c < bd.cell.length; c++) {
				...
				let adc = cell.adjacent, adj = null;
				switch (cell.qans) {
