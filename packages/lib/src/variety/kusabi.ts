...
export const Kusabi = createVariety({
	pid: "kusabi",
	//---------------------------------------------------------
	// マウス入力系
	MouseEvent: {
	...
	AnsCheck: {
		...
		checkProperLetter: function () {
			this.checkLineShape(function (path) {
				const cell1 = path.cells[0], cell2 = path.cells[1], qn1 = cell1.qnum, qn2 = cell2.qnum;
		...
		checkLengthWrong: function () {
			this.checkLineShape(function (path) {
				const cell1 = path.cells[0], cell2 = path.cells[1], qn1 = cell1.qnum, qn2 = cell2.qnum, length = path.length;
