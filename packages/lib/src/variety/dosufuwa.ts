...
//
export const Dosufuwa = createVariety({
	pid: "dosufuwa",
	//---------------------------------------------------------
	// マウス入力系
	MouseEvent: {
	...
	GraphComponent: {
		checkCmp: function () {
			...
			for (let i = 0; i < this.clist.length; i++) {
				if (this.clist[i].qans === 1) { bcnt++; }
				else if (this.clist[i].qans === 2) { icnt++; }
			}
	...
	FileIO: {
		...
		decodeAreaRoom_com: function (isques) {
			const bd = this.board;
			this.readLine();
			const items = this.getItemList(bd.rows);
		...
		encodeAreaRoom_com: function (isques) {
			...
			let data = '';
			for (let c = 0; c < bd.cell.length; c++) {
				const roomid = rooms.indexOf(bd.cell[c].room as any);
	...
	AnsCheck: {
		...
		checkBalloonIsTop: function () {
			this.checkAllCell(function (cell) { const cell2 = cell.adjacent.top; return (cell.qans === 1 && !cell2.isnull && cell2.ques !== 1 && cell2.qans !== 1); }, "cuNotTop");
		},
		checkIronBallIsBottom: function () {
			this.checkAllCell(function (cell) { const cell2 = cell.adjacent.bottom; return (cell.qans === 2 && !cell2.isnull && cell2.ques !== 1 && cell2.qans !== 2); }, "csNotBottom");
		}
	},
