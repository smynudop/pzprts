...
//
export const Kinkonkan = createVariety({
	pid: "kinkonkan",
	//---------------------------------------------------------
	// マウス入力系
	MouseEvent: {
		isclearflash: false,
		inputModes: { edit: ['border', 'letter', 'letter-', 'number', 'clear'], play: [] },
		...
		inputedit_onstart: function () {
			const piece = this.getcell_excell(); /* cell or excell */
			const bd = this.board;
		...
		inputclean_excell: function () {
			...
			const exlist = new EXCellList([excell]);
		...
		inputqchar_excell: function () {
			...
			if (excell !== this.cursor.getex()) {
				...
				let val = excell.qchar, isInc = ((this.inputMode === 'letter') === (this.btn === 'left'));
	...
	KeyEvent: {
		...
		key_inputexcell: function (ca: string) {
			const excell = this.cursor.getex(), bd = this.board;
			if ((excell.bx === bd.minbx + 1 || excell.bx === bd.maxbx - 1) &&
				(excell.by === bd.minby + 1 || excell.by === bd.maxby - 1)) { return; }

			const qn = excell.qnum;
			if ('0' <= ca && ca <= '9') {
				const num = +ca, max = excell.getmaxnum();
	...
	Board: {
		...
		searchLight: function (startexcell: EXCell, setlight: boolean): { cnt: number, dest: number } {
			let ccnt = 0, ldata = [];
			for (let c = 0; c < this.cell.length; c++) { ldata[c] = 0; }

			let pos = startexcell.getaddr(), dir = 0;
			...
			while (dir !== 0) {
				...
				const qb = cell.qans, cc = cell.id;
				if (qb === 31) {
					if (dir === 1) { ldata[cc] = (!Number.isNaN({ 4: 1, 1: 1 }[ldata[cc]]) ? 1 : 2); dir = 3; }
					else if (dir === 2) { ldata[cc] = (!Number.isNaN({ 2: 1, 1: 1 }[ldata[cc]]) ? 1 : 4); dir = 4; }
					else if (dir === 3) { ldata[cc] = (!Number.isNaN({ 2: 1, 1: 1 }[ldata[cc]]) ? 1 : 4); dir = 1; }
					else if (dir === 4) { ldata[cc] = (!Number.isNaN({ 4: 1, 1: 1 }[ldata[cc]]) ? 1 : 2); dir = 2; }
				}
				else if (qb === 32) {
					if (dir === 1) { ldata[cc] = (!Number.isNaN({ 5: 1, 1: 1 }[ldata[cc]]) ? 1 : 3); dir = 4; }
					else if (dir === 2) { ldata[cc] = (!Number.isNaN({ 3: 1, 1: 1 }[ldata[cc]]) ? 1 : 5); dir = 3; }
					else if (dir === 3) { ldata[cc] = (!Number.isNaN({ 5: 1, 1: 1 }[ldata[cc]]) ? 1 : 3); dir = 2; }
					else if (dir === 4) { ldata[cc] = (!Number.isNaN({ 3: 1, 1: 1 }[ldata[cc]]) ? 1 : 5); dir = 1; }
				}
			...
			const destec = pos.getex().id;
			if (!!setlight) {
				for (let c = 0; c < this.excell.length; c++) { this.excell[c].qinfo = 0; }
				startexcell.qinfo = 1;
				this.excell[destec].qinfo = 1;
				for (let c = 0; c < this.cell.length; c++) { this.cell[c].qinfo = ldata[c]; }
				this.hasinfo = true;
			}
	...
	Graphic: {
		...
		drawBGCells_kinkonkan: function () {
			const g = this.vinc('cell_back', 'crispEdges');

			const clist = this.range.cells;
			for (let i = 0; i < clist.length; i++) {
				const cell = clist[i], info = cell.error || cell.qinfo;
		...
		getQuesNumberText: function (excell): string {
			let text = "", canum = excell.qchar, num = excell.qnum;
			if (canum === 0) { text = ""; }
			else { text = this.getNumberTextCore_letter(canum <= 52 ? canum : canum - 52); }
			if (num >= 0) { text += num.toString(10); }
			return text;
		},
		getQuesNumberColor: function (excell): string {
	...
	Encode: {
		...
		decodeKinkonkan: function () {
			// 盤面外数字のデコード
			const subint = [];
			let ec = 0, a = 0, bstr = this.outbstr, bd = this.board;
			for (let i = 0; i < bstr.length; i++) {
				const ca = bstr.charAt(i), excell = bd.excell[ec];

				if (this.include(ca, 'A', 'Z')) { subint.push(ec); excell.qchar = Number.parseInt(ca, 36) - 9; }
				else if (this.include(ca, '0', '9')) { subint.push(ec); excell.qchar = Number.parseInt(ca, 36) - 9 + (Number.parseInt(bstr.charAt(i + 1), 10) + 1) * 26; i++; }
				else if (this.include(ca, 'a', 'z')) { ec += (Number.parseInt(ca, 36) - 10); }
			...
			for (let i = a; i < bstr.length; i++) {
				const ca = bstr.charAt(i), excell = bd.excell[subint[ec]];
				if (ca === '.') { excell.qnum = -2; }
				else if (ca === '-') { excell.qnum = Number.parseInt(bstr.substr(i + 1, 2), 16); i += 2; }
				else { excell.qnum = Number.parseInt(bstr.substr(i, 1), 16); }
		...
		encodeKinkonkan: function () {
			let cm = "", cm2 = "", bd = this.board;

			// 盤面外部分のエンコード
			let count = 0;
			for (let ec = 0; ec < bd.excell.length; ec++) {
				let pstr = "", val = bd.excell[ec].qchar, qnum = bd.excell[ec].qnum;
		...
		encodeKinkonkan: function () {
			this.encodeCellExcell(function (obj) {
				if (obj.isnull) { }
					...
					const dir = obj.qchar, qn = obj.qnum;
					const str1 = (dir !== 0 ? "" + dir : "");
					const str2 = (qn !== -1 ? "" + qn : "");
	...
	AnsCheck: {
		...
		checkMirrors: function (type: number, code: string) {
			let d: Record<number, number> = {}, bd = this.board, result = true, errorExcell = null;
			for (let ec = 0; ec < bd.excell.length; ec++) {
				const excell = bd.excell[ec];
				if (typeof d[ec] === "number" || excell.qnum === -1 || excell.qchar === 0) { continue; }
				const ret = bd.searchLight(excell, false), excell2 = bd.excell[ret.dest];
