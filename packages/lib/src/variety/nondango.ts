...
//
export const Nondango = createVariety({
	pid: "nondango",
	//---------------------------------------------------------
	// マウス入力系
	MouseEvent: {
	...
	Encode: {
		...
		decodeBoard_nondango: function () {
			let bstr = this.outbstr, c = 0, i = 0, bd = this.board, twi = [16, 8, 4, 2, 1];
			allloop:
		...
		encodeBoard_nondango: function () {
			let cm = "", count = 0, pass = 0, bd = this.board, twi = [16, 8, 4, 2, 1];
	...
	AnsCheck: {
		...
		checkThreeCircles: function (func: (cell: Cell) => boolean, failcode: string) {
			const bd = this.board
			for (let c = 0; c < bd.cell.length; c++) {
				let cell = bd.cell[c], bx = cell.bx, by = cell.by, clist: CellList, clists = [new CellList<Cell>([cell])];
