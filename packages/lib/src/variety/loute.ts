...
//
export const Loute = createVariety({
	pid: "loute",
	//---------------------------------------------------------
	// マウス入力系
	MouseEvent: {
	...
	AreaRoomGraph: {
		...
		setExtraData: function (component) {
			...
			if (subclist.length === 0 || (dl.cols * dl.rows !== dl.cnt) || ((d.cols - 1) !== dl.cols) || ((d.rows - 1) !== dl.rows)) {
				...
				/* 端のセル */
				const isUL = (d.x1 === dl.x1 && d.y1 === dl.y1), isUR = (d.x2 === dl.x2 && d.y1 === dl.y1),
	...
	Encode: {
		...
		decodeLoute: function () {
			...
			for (i = 0; i < bstr.length; i++) {
				...
				if (this.include(ca, "0", "9") || this.include(ca, "a", "f")) { cell.qdir = Number.parseInt(ca, 16); }
