...
//
export const Hakoiri = createVariety({
	pid: "hakoiri",
	//---------------------------------------------------------
	// マウス入力系
	MouseEvent: {
	...
	KeyEvent: {
		...
		key_hakoiri: function (ca: string) {
	...
	Graphic: {
		...
		drawQnumMarks: function () {
			const g = this.vinc('cell_mark', 'auto');
	...
	AnsCheck: {
		...
		checkAroundMarks: function () {
			...
			for (let c = 0; c < bd.cell.length; c++) {
				const cell = bd.cell[c], num = cell.getNum();
				if (num < 0) { continue; }
				let target: Cell, clist = new CellList();
				const func = function (cell: Cell) { return (!cell.isnull && num === cell.getNum()); };
				// 右・左下・下・右下だけチェック
				clist.add(cell);
