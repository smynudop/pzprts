...
//
export const Hanare = createVariety({
	pid: "hanare",
	//---------------------------------------------------------
	// マウス入力系
	MouseEvent: {
		...
		inputqnum_hanare: function () {
			...
			const result = cell.setNum_hanare(cell.room.clist.length);
			if (result !== null) {
	...
	Cell: {
		...
		setNum_hanare: function (val: number) {
			if (val >= 0) {
				if (val > this.getmaxnum()) { return null; }
				const puzzle = this.puzzle, issingleansnum = puzzle.execConfig('singlenum');
				let clist = this.room.clist, cell2 = null;
	...
	AnsCheck: {
		...
		checkDiffNumber: function () {
			let cell: Cell | null, num: number, distance: number;
			let result = true, bd = this.board;
			const eachcell = (cell2: Cell) => {
				distance++;
				if (!cell2.isNum()) { /* nop */ }
				else if (!cell2.isValidNum()) { cell = null; }
				else {
			...
			for (let bx = bd.minbx + 1; bx <= bd.maxbx - 1; bx += 2) {
				cell = null;
				for (let by = bd.minby + 1; by <= bd.maxby - 1; by += 2) {
		...
		checkAnsNumberAndSize: function () {
			...
			for (let r = 0; r < rooms.length; r++) {
				let clist = rooms[r].clist, num = -1;
				for (let i = 0; i < clist.length; i++) { if (clist[i].isNum()) { num = clist[i].getNum(); break; } }
