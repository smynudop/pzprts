...
//
export const Tasquare = createVariety({
	pid: "tasquare",
	//---------------------------------------------------------
	// マウス入力系
	MouseEvent: {
	...
	Graphic: {
		...
		drawCellSquare: function () {
			const g = this.vinc('cell_square', 'crispEdges', true);

			const rw = this.bw * 0.8 - 1;
			const rh = this.bh * 0.8 - 1;
	...
	AnsCheck: {
		...
		checkNumberSquare: function (flag: boolean, code: string) {
			const bd = this.board;
			for (let c = 0; c < bd.cell.length; c++) {
				const cell = bd.cell[c];
				if ((flag ? (cell.qnum < 0) : (cell.qnum === -1))) { continue; }
				const clist = new CellList(), adc = cell.adjacent;
