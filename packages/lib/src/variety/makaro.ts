...
//
export const Makaro = createVariety({
	pid: "makaro",
	//---------------------------------------------------------
	// マウス入力系
	MouseEvent: {
		...
		inputqnum_main: function (cell: Cell) {	// オーバーライド
		...
		inputshade_preqnum: function (cell: Cell): boolean {
			let val = null;
	...
	KeyEvent: {
		...
		key_inputcell_makaro_edit: function (cell: Cell, ca: string) {
			let retval = false;
	...
	AreaRoomGraph: {
		enabled: true,
		isnodevalid: function (cell): boolean { return (cell.ques === 0); }
	},
	...
	Encode: {
		...
		decodeMakaro: function () {
			...
			for (i = 0; i < bstr.length; i++) {
				...
				if (this.include(ca, "0", "9")) { cell.qnum = Number.parseInt(ca, 10) + 1; }
				else if (ca === '-') { cell.qnum = Number.parseInt(bstr.substr(i + 1, 2), 10) + 1; i += 2; }
				else if (ca >= 'a' && ca <= 'e') { cell.ques = 1; cell.qdir = Number.parseInt(ca, 36) - 10; }
		...
		encodeMakaro: function () {
			...
			for (let c = 0; c < bd.cell.length; c++) {
				let pstr = "", cell = bd.cell[c], qn = cell.qnum;
	...
	AnsCheck: {
		...
		checkPointAtBiggestNumber: function () {
			for (let c = 0; c < this.board.cell.length; c++) {
				...
				let list = cell.getdir4clist(), maxnum = -1, maxdir: IDir | 0 = cell.NDIR;
				let dupnum = false, isempty = false, invalidarrow = true;
				for (let i = 0; i < list.length; i++) {
					const num = list[i][0].getNum();
