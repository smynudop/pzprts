...
//
export const Firefly = createVariety({
	pid: "firefly",
	//---------------------------------------------------------
	// マウス入力系
	MouseEvent: {
	...
	Graphic: {
		...
		drawFireflyDots: function () {
			const g = this.vinc('cell_firefly', 'auto');

			g.fillStyle = this.quescolor;
			const rsize = this.cw * 0.40;
			const rsize3 = this.cw * 0.10;
			...
			for (let i = 0; i < clist.length; i++) {
				...
				if (cell.qnum !== -1 && cell.qdir !== cell.NDIR) {
					let px = cell.bx * this.bw, py = cell.by * this.bh;
					switch (cell.qdir) {
	...
	AnsCheck: {
		...
		checkFireflyBeam: function () {
			...
			for (let c = 0; c < bd.cell.length; c++) {
				const cell = bd.cell[c], dir = cell.qdir;
		...
		checkConnectPoints: function () {
			this.checkLineShape(function (path) {
				const cell1 = path.cells[0], cell2 = path.cells[1];
		...
		checkCurveCount: function () {
			this.checkLineShape(function (path) {
				const cell1 = path.cells[0], cell2 = path.cells[1];
				const qn = ((path.dir1 === cell1.qdir) ? cell1 : cell2).qnum;
