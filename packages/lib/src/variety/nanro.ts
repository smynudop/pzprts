...
//
export const Nanro = createVariety({
	pid: "nanro",
	//---------------------------------------------------------
	// マウス入力系
	MouseEvent: {
	...
	AreaRoomGraph: {
		...
		setNumOfRoom: function (component: NanroGraphComponent) {
			const clist = component.clist;
			const nums = [];
			let numkind = 0, filled = -1;
			for (let i = 0; i < clist.length; i++) {
				const num = clist[i].getNum();
				if (num !== -1) {
					if (!(num in nums)) { numkind++; filled = num; nums[num] = 1; }
					else { nums[num]++; }
				}
			}
	...
	AnsCheck: {
		...
		checkAllErrorRoom: function (evalfunc: (area: NanroGraphComponent) => boolean, code: string) {
...
type NanroGraphComponent = GraphComponent & {
	// AreaRoomGraphの拡張
	number: number; // 部屋の数字
	numcnt: number; // 数字の個数
	numkind: number; // 数字の種類数
}