...
//
export const Juosan = createVariety({
	pid: "juosan",
	//---------------------------------------------------------
	// マウス入力系
	MouseEvent: {
	...
	AnsCheck: {
		...
		checkRowsColsSeparate: function (
			evalfunc: (clist: CellList, info: any) => boolean,
			categoryfunc: (cell: Cell) => number,
			code: string
		) {
			let result = true, bd = this.board, info: any;
			allloop: do {
				/* 横方向サーチ */
				info = { isvert: false };
				for (let by = 1; by <= bd.maxby; by += 2) {
					for (let bx = 1; bx <= bd.maxbx; bx += 2) {
						let val = categoryfunc(bd.getc(bx, by)), tx = bx;
				...
				for (let bx = 1; bx <= bd.maxbx; bx += 2) {
					for (let by = 1; by <= bd.maxby; by += 2) {
						let val = categoryfunc(bd.getc(bx, by)), ty = by;
		...
		isParallelCount: function (clist: CellList, info: any) {
		...
		checkMajorityBarCount: function (isover: boolean, code: string) {
			let result = true, rooms = this.board.roommgr.components;
			for (let r = 0; r < rooms.length; r++) {
				const area = rooms[r];
				if (!area.top.isValidNum()) { continue; }
				let clist = area.clist, vcount = 0, hcount = 0, count = 0;
				for (let i = 0; i < clist.length; i++) {
