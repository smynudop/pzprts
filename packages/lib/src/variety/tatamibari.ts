...
//
export const Tatamibari = createVariety({
	pid: "tatamibari",
	//---------------------------------------------------------
	// マウス入力系
	MouseEvent: {
		inputModes: { play: ['border', 'subline'], edit: [] },
	...
	KeyEvent: {
		...
		key_inputMarks: function (ca: string) {
	...
	BoardExec: {
		adjustBoardData: function (key, d) {
			if (key & this.TURN) {
				const tques: Record<number, number> = { 2: 3, 3: 2 };
				const clist = this.board.cellinside(d.x1, d.y1, d.x2, d.y2);
				for (let i = 0; i < clist.length; i++) {
					const cell = clist[i], val = tques[cell.qnum];
	...
	Graphic: {
		...
		drawMarks: function () {
			...
			const lm = Math.max(this.cw / 12, 3) / 2;	//LineWidth
			const lp = this.bw * 0.70;				//LineLength
			g.fillStyle = this.quescolor;

			const clist = this.range.cells;
			for (let i = 0; i < clist.length; i++) {
				const cell = clist[i], qn = cell.qnum;
