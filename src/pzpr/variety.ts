...
type GenreOption =
	[number, number, string, string, parent: string, option: any]
	| [number, number, string, string, parent: string]
	| [number, number, string, string]
function Genre(pzprid: string, datalist: GenreOption) {
	return {
		valid: true,
		pid: pzprid,		/* パズルID */
		script: (!!datalist[4] ? datalist[4] : pzprid),	/* スクリプトファイル(クラス) */
		ja: datalist[2],	/* 日本語パズル名 */
		en: datalist[3],	/* 英語パズル名 */
		exists: {
			pzprapp: !!datalist[0],
			kanpen: !!datalist[1],
			pencilbox: !!datalist[1]
		},
		alias: (!!datalist[5] ? datalist[5] : {}),
		urlid: (!!datalist[5] ? datalist[5].pzprurl : pzprid),
		kanpenid: ((!!datalist[1] && !!datalist[5]) ? (datalist[5].kanpen || pzprid) : ''),

	}
}

const varietylist: Record<string, GenreOption> = {
...
export const createVariety = function () {
	...
	variety.info = _info
	variety.toPID = toPID
	variety.exists = function (name: string) {
